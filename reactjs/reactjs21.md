...
- Comienza con `return <AppLab />` — explora los 5 labs de Ant Design usando el selector
- Cuando termines el lab, comenta `return <AppLab />` y descomenta `return <AppHome />` — ve el dashboard real
- Navega entre "/" y "/about" con el `Menu` lateral — observa el ítem activo resaltado por Ant Design
- Vuelve a la Fase 1 cuando quieras practicar un componente específico de nuevo
