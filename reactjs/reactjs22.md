...
- Pasa el cursor sobre `<Button variant="contained">` — observa el efecto hover que viene incluido en el tema de MUI
- Cambia `color="primary"` por `color="secondary"` en un botón — ve el cambio de color del tema
- Observa el botón `loading` — el spinner aparece automáticamente sin `useState` ni `CircularProgress`
- Pasa el cursor sobre el `<IconButton>` de papelera — aparece el `Tooltip` "Eliminar elemento"
- Cambia `size="large"` del botón Grande a `size="small"` — observa cómo MUI gestiona el padding automáticamente
...
- Observa los 4 `Alert` con distintas `severity` — cada uno tiene icono y color diferente automáticamente
- Las alertas con `<AlertTitle>` muestran un título en negrita sin clases CSS adicionales
- Haz clic en la X del `Alert` con `Collapse` — desaparece con animación suave gracias a `Collapse`
- Haz clic en "Mostrar alerta" — vuelve a aparecer con la misma animación suave
- Cambia `severity="info"` del `Alert` con `Collapse` a `severity="warning"` — cambia el color y el icono
...
- Reduce el ancho de la ventana — el grid cambia de 3 columnas (`md:4`) a 2 (`sm:6`) a 1 (`xs:12`)
- Cambia `active: false` del tercer producto a `true` — el Chip cambia de "Inactivo" a "Activo" (verde)
- Cambia `spacing={2}` del `Grid container` a `spacing={4}` — aumenta el espacio entre Cards
- Observa `CardMedia` con `onError` — si la imagen falla, muestra el placeholder de `placehold.co`
...
- Envía el formulario vacío — los campos muestran error con `helperText` en rojo sin CSS extra
- Escribe un email sin `@` — aparece "Email inválido" debajo del campo
- Completa todos los campos correctamente — aparece el `Alert` verde de éxito por 3 segundos
- Abre el `TextField select` de Rol — el menú desplegable tiene el mismo estilo que el resto del formulario
- Escribe algo en el campo Nombre y luego bórralo — el mensaje de error desaparece en tiempo real
...
- Escribe "teclado" en el buscador — filtra en tiempo real con el icono de lupa integrado en `InputAdornment`
- Haz clic en la cabecera "Precio" — el `TableSortLabel` cambia entre `asc` y `desc` con flecha visual
- Escribe "xyz" — aparece la fila "Sin resultados" dentro de la tabla
- Agrega un producto al array `PRODUCTS` con `active: true` — aparece en la tabla con Chip verde
- Cambia `Table size="small"` a `size="medium"` — los rows se hacen más altos
...
- Haz clic en cada pestaña del `AppBar` — navega entre los 5 laboratorios sin recargar la página
- Observa el `indicatorColor="secondary"` — el indicador de la pestaña activa usa el color secondary del tema
- Cambia el valor inicial de `useState<LabKey>(0)` a `useState<LabKey>(3)` — el lab arranca en "Form"
- Cuando termines el lab, pasa a la Fase 2: cambia `return <AppLab />` a `return <AppHome />` en `App.tsx`
...
- Reduce el ancho de la ventana por debajo de `md` (900px) — el `Drawer` permanente desaparece y aparece el `MenuIcon`
- Haz clic en el `IconButton` con el ícono de menú (móvil) — el Drawer temporal se abre con animación
- Haz clic en "Dashboard" y luego en "Acerca de" — observa el resaltado `isActive` en el NavLink activo
- Cambia `DRAWER_WIDTH = 240` a `300` — el sidebar se ensancha en escritorio
...
- Activa `AppHome` en `App.tsx` y abre el Dashboard — se cargan los productos desde la API real
- Cambia entre la pestaña "Cards" y "Tabla" — los mismos datos se muestran en dos formatos distintos
- Escribe en el buscador — la consulta se envía con `useCallback` al cambiar y filtra en la API
- Cambia el selector "Por página" de 5 a 10 — la paginación se actualiza y se cargan más productos
- Usa los botones de `Pagination` para navegar entre páginas — observa el spinner `CircularProgress` durante la carga
- Observa la URL que muestra la página: `/products/?page=1&page_size=5` actualizada en el subtítulo
...
- Activa `AppHome` en `App.tsx` — ve el dashboard completo con sidebar, AppBar y contenido
- Navega entre "/" y "/about" desde el sidebar — el Drawer resalta el link activo con fondo azul
- En móvil (< 900px), abre el sidebar con el ícono de menú — el Drawer temporal se cierra al navegar
- Añade una tercera ruta `<Route path="/settings" element={<div>Settings</div>} />` y un NavLink en el array `NAV` de `MuiShell`
...
- Comienza con `return <AppLab />` — explora los 5 labs usando las pestañas del AppBar
- Cuando termines el lab, comenta `return <AppLab />` y descomenta `return <AppHome />` — ve el dashboard real
- Verifica que el `ThemeProvider` en `main.tsx` afecta a ambas fases sin cambios adicionales
